    ...
    <div>
        <section class="hero is-info welcome is-small">
            <div class="hero-body">
                <div class="container">
                    ...
                    <h2 class="subtitle">
                        Wszystkie produkty dostępne w sklepie
                    </h2>
                </div>
            </div>
        </section> 
            ...
            <table class="table is-striped is-fullwidth">
                ...
                <tbody>
                    <tr v-for="product in products" v-bind:key="product.id">
                        <th>{{ product.id }}</th>
                        <td>{{ product.name }}</td>
                        <td :class="{ 'not-on-sale': (product.discountPrice < product.price && product.discountPrice != 0) }">{{ product.price }} zł</td>
                        <td class="not-on-sale" :class="{ 'is-on-sale': product.discountPrice != 0 }">{{ product.discountPrice }} zł</td>
                        <td>
                            <span v-for="(size, index) in product.sizes" :key="index">{{ size.sizeName + '-' + size.pivot.quantity + ' ' }}</span>
                        </td>
                        <td><input type="checkbox" :id="product.id" :value="product.id" v-model="checkedProducts" @change="showProductEdit = false"></td>
                    </tr>    
                </tbody>                                         
            </table>    
        ...
        <section class="product-edit" v-show="showProductEdit">
            <form>
                ...
                <div class="field">                    
                    <div class="control">
                        <input class="input" type="text" placeholder="Nazwa" v-model="productName">
                    </div>
                </div>
                <label class="label">Krótki opis</label>
                <div class="field">
                    <div class="control">
                        <textarea class="textarea has-fixed-size" placeholder="Krótki opis produktu" rows="3" v-model="shortDescription"></textarea>
                ...
                <div class="field">
                    <div class="control">
                        <textarea class="textarea has-fixed-size" placeholder="English short description" rows="3" v-model="shortEnglishDescription"></textarea>
                        <p class="is-pulled-right char-counter">{{ shortEnglishDescription.length }}/{{ maxChars }}</p>
                    </div>
                </div>
                <br>
                    ...
                    <div class="field-body">                        
                        <div class="field">
                            <div class="control is-expanded">
                                <input class="input" type="text" placeholder="Cena" v-model="price">                                                             
                            </div>
                        </div>                        
                        <div class="field">
                            <div class="control is-expanded">
                                <input class="input" type="text" placeholder="Cena promocyjna" v-model="discountPrice">                            
                            </div>
                        </div>                        
                    </div>
                </div>
                <label class="label">Długi opis</label>
                 <div class="field">
                    <div class="control">
                        <textarea class="textarea has-fixed-size" placeholder="Długi opis produktu" rows="8" v-model="longDescription"></textarea>                        
                    </div>
                </div>
                <label class="label">Długi opis po angielsku</label>
                 <div class="field">
                    <div class="control">
                        <textarea class="textarea has-fixed-size" placeholder="English long description" rows="8" v-model="longEnglishDescription"></textarea>                        
                    </div>
                </div>
                <label class="label">Opis materiałów</label>
                 <div class="field">
                    <div class="control">
                        <textarea class="textarea has-fixed-size" placeholder="Opis materiałów" rows="3" v-model="materialsDescription"></textarea>                        
                    </div>
                </div>
                <label class="label">Opis materiałów po angielsku</label>
                 <div class="field">
                    <div class="control">
                        <textarea class="textarea has-fixed-size" placeholder="English materials description" rows="3" v-model="englishMaterials"></textarea>                        
                    </div>
                </div>
                ...
                <div class="field" v-for="(size, index) in sizes" :key="index">
                    <div class="control">
                        <label class="checkbox">
                            <input type="checkbox" v-model="sizesChecked[index]">
                            {{ size.sizeName }}
                        </label>                    
                    </div>
                    <div class="control">
                        <input class="input" type="text" placeholder="Stan magazynowy" v-if="sizesChecked[index]" v-model="quantities[index]">                            
                    </div>
                </div>                                

                <p class="buttons">
                    <a class="button is-success" @click="submitEditedProduct(editedProduct)">
                        <span class="icon is-small">
                            <i class="fas fa-save"></i>
                        </span>
                        <span>Zapisz</span>
                    </a>
                    <a class="button is-danger is-outlined" @click="showProductEdit = false">
                        <span>Anuluj</span>
                        <span class="icon is-small">
                            <i class="fas fa-times"></i>
                        </span>
...
export default {
    ...
    methods: {
        ...
        submitEditedProduct(editedProduct) {
            axios.put('/products/' + editedProduct.id, {
